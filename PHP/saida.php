...
class Saida
{
    static function all()
    {
        ...
        $pagina = Paginas("produto", 10);
        $pagina["inicio"] . "<br>";
        $pagina['num_paginas'];

        $sql = "SELECT * FROM saida ORDER BY id DESC LIMIT $pagina[inicio], $pagina[quantidadePorPagina]";
            ...
            <tbody>
                <?php while($data = mysqli_fetch_assoc($query)): ?>
                    <tr id="tr[<?php echo $data['id']?>]" style="padding: 1rem">
                        ...
                        <td><?php echo $data['valor_produto']?></td>
                        <td><?php echo $data['data_saida']?></td>
